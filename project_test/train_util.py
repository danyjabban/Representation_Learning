...
def nt_xent_loss(x, temperature=0.5):
...
def train_no_DDP(net, epochs, batch_size, lr, reg, log_every_n=50):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        total = 0
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            augment_inputs = augment_data(inputs).to(device)
            # del inputs
            optimizer.zero_grad()
            outputs = net(augment_inputs)
            # del augment_inputs
...
def train_nt_xet_class(net, epochs, batch_size, lr, reg, log_every_n=50):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            isnan = sum(torch.isnan(torch.tensor(torch.cat((outputs1.clone().detach(), outputs2.clone().detach())))).to('cpu').numpy().astype(int).flatten())
            if isnan != 0: 
                print(isnan)
            # del augment_inputs1
            # del augment_inputs2
